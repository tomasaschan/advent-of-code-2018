  ...
  describe "Dec 4" $ do
    ...
    context "parsing" $ do
      it "can parse a wake-up sample" $ do
        ...
        parse sample `shouldBe`
          Event {month = 11, day = 21, hour = 00, minute = 43, what = WakeUp}
      it "can parse a fall-asleep sample" $ do
        let sample = "[1518-11-22 00:04] falls asleep"
        parse sample `shouldBe`
          Event
            {month = 11, day = 22, hour = 00, minute = 04, what = FallAsleep}
    ...
    context "building state matrix" $ do
      it "can build one row" $ do
        ...
        let row =
              [Away] ++
              replicate 3 Awake ++ replicate 39 Asleep ++ replicate 17 Awake
