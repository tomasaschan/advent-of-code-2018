...
solveB input = show answer
 where
  ...
  answer =
    uncurry (*) $ (id >< fst) $ maximumBy (compare `on` (snd . snd)) $ map
      mapper
      guards
...
buildEvent [mo, dd, hh, mm, "wakes up"] = Event { month  = read mo
                                                , day    = read dd
                                                , hour   = read hh
                                                , minute = read mm
                                                , what   = WakeUp
                                                }
buildEvent [mo, dd, hh, mm, "falls asleep"] = Event { month  = read mo
                                                    , day    = read dd
                                                    , hour   = read hh
                                                    , minute = read mm
                                                    , what   = FallAsleep
                                                    }
buildEvent [mo, dd, hh, mm, guard] = Event { month  = read mo
                                           , day    = read dd
                                           , hour   = read hh
                                           , minute = read mm
...
_insertSplitter e = case what e of
...
asleepInMinute guard m =
  _asleepInMinute m . map snd . filter ((==) guard . fst)
