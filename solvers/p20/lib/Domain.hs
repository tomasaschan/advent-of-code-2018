...
connect d x y (Base m) =
    Base
    $ Map.insertWith union (x,y) [d]
    $ Map.insertWith union (walk (x,y) d) [one80 d] m

connectAll :: [Coord] -> Direction -> Base -> Base
connectAll pos d b = foldr (uncurry (connect d)) b pos

merge :: Base -> Base -> Base
merge (Base a) (Base b) = Base $ M.merge M.preserveMissing M.preserveMissing (M.zipWithMatched (const union)) a b
...
instance Show Base where
    show b =
        let Base m = b
            ...
            above :: Int -> String
            above y = "#" ++ intersperse '#' [if door x y N b then '-' else '#' | x <- [minx .. maxx]] ++ "#"
            at :: Int -> String
            at y = intersperse '.' [if door x y E b then '|' else '#' | x <- [minx - 1 .. maxx]]
...
instance Show Explored where
    ...
    show (Explored b e) =
        let Base m = b
            ...
            at y = "# " ++ concat [printf "%s %s" (s x y) (eof x y) | x <- [minx .. maxx]]
...
explore b = Explored b $ explore' Map.empty [((0,0), 0)]
    where
        ...
        explore' m ((p,l):ps)                  =
            let
                m' = Map.insert p l m
                ps'  = nubBy ((==) `on` fst) . sortBy (compare `on` snd) . (<>ps) . fmap (, l+1) . uncurry neighbors p $ b
            in explore' m' ps'
