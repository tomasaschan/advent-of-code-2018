...
solveB input = either show show $ do
  ...
  let opCodes =
        refine IntMap.empty
          . foldl (flip learn) anythingsPossible
          . fst
          $ parsed
...
interpret codes (o, x, y, z) = case IntMap.lookup o codes of
  Just op -> Right (op, x, y, z)
  Nothing ->
    Left
      $  "the interpretation of "
      ++ show o
      ++ " was not determined by the learning steps"
...
refine known partials
  ...
  | otherwise
  = let determined =
            IntMap.map (Set.elemAt 0)
              . IntMap.filter ((== 1) . Set.size)
              $ partials
...
registry =
  fmap fromList $ char '[' *> number `sepBy1` (string ", ") <* char ']'
