...
summedAreaTable n p = _extend 1 1 empty
 where
  at x y prev
    ...
    = p (x, 1) + prev ! (x - 1, 1)
    | otherwise
    = p (x, y) + prev ! (x - 1, y) + prev ! (x, y - 1) - prev ! (x - 1, y - 1)
  _extend x y sofar | x == n && y == n = insert (n, n) (at x y sofar) sofar
...
areaPowerLookup sat (x, y, s) = trace
  (printf
    "looking up power at (%d,%d) with size %d. a=%d, b=%d, c=%d, d=%d, p=%d"
    x
    y
    s
    a
...
superCellWithHighestPower
  :: SummedAreaTable -> [(LargeCellIdentifier)] -> LargeCellIdentifier
