...
tr x y ug f s = 
    trace (draw (ug, f)) $ trace (printf "(%d,%d) %s" x y s)
  
pour :: Underground -> (Underground, Flow)
pour ug = (ug, fall 0 ug 500 1 Map.empty)

fall :: Int -> Underground -> Int -> Int -> Flow -> Flow
fall i     ug x y f | y > maximum (Set.map snd ug)                = tr x y ug f (printf "fall 1 %d" i) f
fall i     ug x y f | i > 10                                      = tr x y ug f (printf "fall 6 %d" i) f
fall i     ug x y f | onSurface ug x y f && shouldSettle ug x y f = tr x y ug f (printf "fall 2 %d" i) settle ug x y f
fall i     ug x y f | onSurface ug x y f                          = tr x y ug f (printf "fall 3 %d" i) fall (i+1) ug x y $ spread ug x y f
fall 0     ug x y f                                               = tr x y ug f (printf "fall 4 %d" (0::Int)) fall 1 ug x y $ fall 0 ug x (y+1) f
fall i     ug x y f                                               = tr x y ug f (printf "fall 5 %d" i) Map.insert (x,y) Falling f
...
spread ug x y = spread' Pos x . spread' Neg x
    where
        ...
        spread' d x' f                                                   = 
          let f' = fall 0 ug x' y f
          in tr x' y ug f' (printf "spread 3 %d" $ dx d) f'


settle :: Underground -> Int -> Int -> Flow -> Flow
settle ug x y = settle' Pos x . settle' Neg x
    where
        settle' d x' f | (x' + dx d, y) `elem` ug = tr x' y ug f' (printf "settle 1 %d" $ dx d) f'
...
shouldSettle ug x y f | onSurface ug x y f = walled Pos ug x y f && walled Neg ug x y f
