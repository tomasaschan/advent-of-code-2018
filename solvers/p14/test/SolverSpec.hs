...
spec = describe "Dec 14" $ do
  it "has a correct initial state" $ do
    initial `shouldBe` State { recipies = (fromList $ reverse [3, 7])
                             , elf1     = 0
                             , elf2     = 1
                             }
  ...
  it "calculates a correct first step" $ do
    next initial `shouldBe` State { recipies = fromList $ reverse [3, 7, 1, 0]
                                  , elf1     = 0
                                  , elf2     = 1
                                  }
  it "calcualtes a correct second step" $ do
    ...
    let s1 = State { recipies = fromList $ reverse [3, 7, 1, 0, 1, 0]
                   , elf1     = 4
                   , elf2     = 3
                   }
    next s0 `shouldBe` s1
