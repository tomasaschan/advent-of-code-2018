...
isAtEq p = isEq . current p
 where
  isEq (Op (Eq (Register _) (Register _) _)) = True
  isEq _ = False
...
patch (Eq (Register Five) (Register Zero) Three) = Custom f
 where
  f s =
    let i  = get (Register Five) s
        f' = if i `notElem` seen s
          then see i
            . flip apply (Op (Eq (Register Five) (Register Zero) Three))
          else withIp (-10) -- terminate
    in  f' s
patch i = Op i

b :: (Register, [Instruction]) -> Int
b = fromMaybe 0 . lastF . uncurry (runUntil terminated) . state . second
  (fmap patch)
