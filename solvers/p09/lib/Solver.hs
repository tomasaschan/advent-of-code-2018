...
solve n =
  show .
  maximum .
  scores . head . filter gameOver . iterate play . scale . initial . parse
  where
    scale s = s {lastMarble = n * lastMarble s}
...
parse = _tuplify . _parse
  where
    _parse x = extracted
      where
        parsed =
          Parse.digits x >>= Parse.expect " players; last marble is worth " >>=
          Parse.digits >>=
          Parse.expect " points"
