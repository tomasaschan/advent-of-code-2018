...
spec = describe "Dec 9" $ do
  ...
  context "a" $ do
    it "can parse the input" $ do
      parse "428 players; last marble is worth 70825 points"
        `shouldBe` (428, 70825)
    ...
    it "can create the initial state" $ do
      initial (9, 25) `shouldBe` State { marbles    = ML ([], [0])
                                       , toBePlaced = 1
                                       , toPlay     = 0
    ...
    it "can place a regular marble" $ do
      let input = State { marbles    = ML ([8, 0], [4, 2, 5, 1, 6, 3, 7])
                        , toBePlaced = 9
                        , toPlay     = 8
                        , scores     = [0, 0, 0, 0, 0, 0, 0, 0, 0]
                        , lastMarble = 25
                        }
      let expected = State { marbles    = ML ([2, 4, 8, 0], [9, 5, 1, 6, 3, 7])
                           , toBePlaced = 10
                           , toPlay     = 0
                           , scores     = [0, 0, 0, 0, 0, 0, 0, 0, 0]
                           , lastMarble = 25
    ...
    it "can solve the examples" $ do
      ...
      solveA ["13 players; last marble is worth 7999 points"]
        `shouldBe` "146373"
