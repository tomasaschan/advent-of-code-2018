...
dependencies'
  ...
  -> BState
  -> BState
dependencies' f (BState { now = t, inProgress = ip, dependencies = d, workers = n })
...
initial n extra input = BState { now          = 0
                               , inProgress   = []
                               , dependencies = parse vx input
                               , workers      = n
                               }
 where
  vx s = (s, duration s)
...
completeWork s = inProgress' removeFromInProgress
  $ dependencies' removeFromDependencies s
 where
