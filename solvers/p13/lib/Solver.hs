...
import           Data.Map               (Map, adjust, empty, fromList, insert,
                                         keys, lookup, mapMaybe, toList)
import           Data.Maybe             (fromJust, fromMaybe, isJust)
import           GHC.Exts               (the)
import           Prelude                hiding (lookup)

import           Data.Foldable.Extended (maximums)
import           Data.List.Extended     (hasDuplicates, readingOrder)
...
solveB input =
  out . position . the . head . simulateUntil 100000 oneCarLeft (tickB tracks) $
  initial
  where
    out (x, y) = show x ++ "," ++ show y
    (tracks, initial) = parse input
...
tickB tracks =
  fmap snd .
  toList .
  moveAll 0 . fromList . zip [0 ..] . sortBy (readingOrder `on` position)
  where
...
rotate tracks pos d t = _rotate tr d t
  where
    ...
    _rotate _ fc _ =
      ...
      show pos ++ ": " ++ show tr ++ "/" ++ show fc ++ "/" ++ show t
