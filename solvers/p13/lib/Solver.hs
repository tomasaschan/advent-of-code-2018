...
import           Data.List.Extended             ( hasDuplicates
                                                , readingOrder
                                                )
...
solveA input =
  out . findCollision . head . simulateUntil 10000 hasCollision tick $ initial
 where
  ...
  tick cars = moveAll cars'
   where
    ...
    moveAll [] = []
    moveAll (c : cs) | hasCollision (c : cs) = (c : cs)
...
solveB input =
  out
    ...
    . the
    . head
    . simulateUntil 100000 oneCarLeft (tickB tracks)
    $ initial
 where
  out (x, y) = show x ++ "," ++ show y
  (tracks, initial) = parse input
...
tickB tracks = fmap snd . toList . moveAll 0 . fromList . zip [0 ..] . sortBy
  (readingOrder `on` position)
 where
  moveAll :: Int -> Map Int Car -> Map Int Car
  moveAll i cs | (maximum . keys $ cs) < i = cs
...
dedupe = fromList . concat . filter ((==) 1 . length) . groups . toList
 where
  ...
  groups = groupBy ((==) `on` pos) where pos (_, (p, _, _)) = p

trackAt :: Tracks -> Position -> Track
trackAt tracks pos = case lookup pos tracks of
  Just t  -> t
...
rotate tracks pos d t = _rotate tr d t
 where
  ...
  _rotate _ fc _ =
    error
      $  "unsupported track/direction/turn combo at "
      ++ show pos
      ++ ": "
      ++ show tr
      ++ "/"
 ...
 where
  ...
  parseCoord y (x, c) = (tracks', cars')
   where
    ...
    cars' =
      fmap (initCarAt (x, y)) . fmap fromJust . filter isJust $ [carFor c]
