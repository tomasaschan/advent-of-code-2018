...
import qualified Data.List  as List (filter, map, maximumBy, minimumBy, nub)
import           Data.Map   (Map, (!))
import qualified Data.Map   as Map (elems, fromList, insert, keys, lookup, map,
                                    mapMaybe)
import           Data.Maybe
...
updateCoord (Cogrid g') candidate (Grid g) here =
  Grid $ Map.insert here winners g
  where
    winners =
      case toBeat (Grid g) here of
        [] -> [candidate]
        b:bs
          | elem candidate (b : bs) -> b : bs
          | otherwise ->
            case closerTo here (g' ! candidate) (g' ! b) of
              LT -> candidate : []
              EQ -> candidate : b : bs
              GT -> b : bs
...
infinites (Grid g) =
  List.nub $
  concat $ filter ((==) 1 . length) $ catMaybes $ map (flip Map.lookup g) edges
  where
    (x, y) = size (G (Grid g))
    upper = [(x', 0) | x' <- [0 .. x]]
    lower = [(x', y) | x' <- [0 .. x]]
...
largestArea (Grid g) =
  ...
  List.filter ((/=) '.') $ List.nub $ Map.elems $ Map.mapMaybe single g
