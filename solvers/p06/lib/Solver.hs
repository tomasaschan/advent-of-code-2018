...
coord input = (x, y)
 where
  parsed =
    Parse.digits input >>= Parse.drop ',' >>= Parse.drop ' ' >>= Parse.digits
  [x, y] = Parse.extract (map (read :: String -> Int)) parsed

type Point = (Coord, [Char])

points :: [String] -> [Point]
points = flip zip cs . map coord where cs = map pure ['A' ..]
...
showGrid (maxX, maxY) (Grid g) = s
 where
  at x y = case Map.lookup (x, y) g of
    Just c | c == [head c] -> head c
           | otherwise     -> 'X'
    Nothing -> '.'
...
size :: GCG -> Size
size (G (Grid g)) =
  ((+) 1 $ maximum $ map fst $ Map.keys g, maximum $ map snd $ Map.keys g)
size (CG (Cogrid cg)) =
  (((+) 1 $ maximum $ map fst $ Map.elems cg, maximum $ map snd $ Map.elems cg))

distance :: Coord -> Coord -> Int
distance (x, y) (x', y') = abs (x - x') + abs (y - y')

closerTo :: Coord -> Coord -> Coord -> Ordering
closerTo target a b | distance target a < distance target b = LT
...
toBeat (Grid g) c = case Map.lookup c g of
  Just cs -> cs
  Nothing -> []
...
updateCoord (Cogrid g') candidate (Grid g) here = Grid
  $ Map.insert here winners g
 where
...
infinites :: Grid -> [Char]
infinites (Grid g) =
  List.nub $ concat $ filter ((==) 1 . length) $ catMaybes $ map
    (flip Map.lookup g)
    edges
 where
  (x, y) = size (G (Grid g))
...
largestArea (Grid g) =
  List.maximumBy (compareAreas (Grid g))
    $ List.filter ((/=) '.')
    $ List.nub
    $ Map.elems
    $ Map.mapMaybe single g

sizeOf :: Grid -> Char -> Int
sizeOf (Grid g) c =
  length $ filter ((==) c) $ Map.elems $ Map.mapMaybe single g
