...
spec =
  describe "Dec 15" $ do
    context "parsing" $ do
      ...
      it "parses units correctly" $ do parseUnits 3 input `shouldBe` units
      it "parses map correctly" $ do parseDungeon input `shouldBe` dungeon
    ...
    context "pathfinding" $ do
      it "chooses the correct target square" $ do
        ...
        let targets =
              foldMap (inRangeOf . position) . filter ((==) Goblin . race) $
              units
