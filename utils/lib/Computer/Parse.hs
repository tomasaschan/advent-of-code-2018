...
initializer =
  initializer' "0" Zero
    ...
    <|> initializer' "4" Four
    <|> initializer' "5" Five
  where initializer' s r = try (string "#ip " >> string s >> return r)
 ...
 where
  anyInstruction =
    instruction' Add "addr" register (Register <$> register) register
      ...
      <|> try
            (   (string "setr" >> return Set)
            <*> (string " " *> (Register <$> register))
            <*> (string " " *> number *> string " " *> register)
            )
      <|> try
            (   (string "seti" >> return Set)
