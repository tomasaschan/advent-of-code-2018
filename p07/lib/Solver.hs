...
instance Show BState where
  show (BState {now = t, inProgress = work}) =
    printf "% 5d   %s" t (intercalate "," $ fmap fst work)

now' :: (Int -> Int) -> BState -> BState
now' f (BState {now = t, inProgress = ip, dependencies = d, workers = n}) =
  BState {now = f t, inProgress = ip, dependencies = d, workers = n}

inProgress' :: ([(String, Int)] -> [(String, Int)]) -> BState -> BState
inProgress' f (BState {now = t, inProgress = ip, dependencies = d, workers = n}) =
  BState {now = t, inProgress = f ip, dependencies = d, workers = n}

dependencies' ::
     (DependencyTree (String, Int) -> DependencyTree (String, Int))
  -> BState
  -> BState
dependencies' f (BState { now = t
                        , inProgress = ip
                        , dependencies = d
                        , workers = n
                        }) =
  BState {now = t, inProgress = ip, dependencies = f d, workers = n}

solveB :: Int -> Int -> [String] -> String
solveB n extra =
  show . now . head . filter isDone . iterate next . initial n extra

initial :: Int -> Int -> [String] -> BState
initial n extra input =
  BState {now = 0, inProgress = [], dependencies = parse vx input, workers = n}
  where
    vx s = (s, duration s)
...
next s
  ...
  | now s > 1000 =
    error
      ("an infinite loop seems to have been entered with current state: " ++
       show s)
  | otherwise =
    error
      ("no available (implemented) actions for the current state: " ++ show s)

available :: BState -> Int
available (BState {workers = n, inProgress = ip, dependencies = _, now = _}) =
  workersAvailable
  where
...
assignWork s = inProgress' ((++) todo) s
  where
    ongoing = fmap fst . inProgress $ s
    pendingWork =
      filter (not . flip elem ongoing . fst) . ready . dependencies $ s
...
completeWork s =
  inProgress' removeFromInProgress $ dependencies' removeFromDependencies s
  where
...
parse f = fromArcsList . fmap parseStep
  where
    parseStep step = arc
      where
        parsed =
          Parse.expect "Step " step >>= Parse.char >>=
