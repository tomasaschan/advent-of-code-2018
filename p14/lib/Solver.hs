...
solveA = unwords . fmap (solve . read)
  where
    solve n = concat . fmap show . drop n . take' (n + 10) $ initial
...
solveB = unwords . fmap solve
  where
    ...
    solve n =
      show .
      prefixLength n .
      recipies . Prelude.head . filter (isFinal (read n)) . iterate next $
      initial
    ...
    prefixLength n rs
      ...
      | otherwise =
        error "trying to get prefix length when it is not a final state"
      where
        ds = fmap (read . (: "")) . reverse $ n
    isFinal :: Int -> State -> Bool
    isFinal n s =
      let ds = reverse $ digits 10 n
          rs = recipies s
...
next :: State -> State
next s =
  let recipies' = append (nextRecipies s) (recipies s)
