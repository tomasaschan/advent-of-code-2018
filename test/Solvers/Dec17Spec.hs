module Solvers.Dec17Spec where

import Data.Either.Extended
import Text.ParserCombinators.Parsec (parse)
import Test.Hspec

import Solvers.Dec17
...
spec = describe "Dec 17" $ do
  context "parsing" $ do
    ...
    it "parses a vertical vein" $ do
      parse vertical "vert" "x=426, y=326..342" `shouldBe` Right ([426], [326..342])

    it "parses sample input" $ do
      let puzzleInput = ["x=495, y=2..7","y=7, x=495..501","x=501, y=3..7","x=498, y=2..4","x=506, y=1..2","x=498, y=10..13","x=504, y=10..13","y=13, x=498..504"]
      let parsed = foldAll . fmap (parse vein "vein") $ puzzleInput
      let expected = [([495], [2..7]),([495..501], [7]),([501], [3..7]),([498], [2..4]),([506], [1..2]),([498], [10..13]),([504], [10..13]),([498..504], [13])]
