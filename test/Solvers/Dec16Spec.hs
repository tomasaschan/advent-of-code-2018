...
spec = describe "Dec 16" $ do
  context "parsing" $ do
    ...
    it "correctly parses a full sample" $ do
      ...
      let expected = Sample { before = fromList [2,3,2,2], opCode = 15, args = (3,2,2), after = fromList [2,3,4,2] }

      parse sample "sample" input `shouldBe` Right expected

  it "counting matches" $ do
    let s = Sample { before = fromList [3,2,1,1], opCode = 9, args = (2,1,2), after = fromList [3,2,2,1] }
