module Solvers.Dec5Spec where
  ...
  spec = describe "Dec 5" $ do
    context "examples for a" $ do
      it "'aA' -> ''" $ do
        annihilate "aA" `shouldBe` ""
      it "'abBA' -> ''" $ do
        annihilate "abBA" `shouldBe` ""
      it "'abAB' -> 'abAB'" $ do
        annihilate "abAB" `shouldBe` "abAB"
      it "'aabAAB' -> 'aabAAB'" $ do
        annihilate "aabAAB" `shouldBe` "aabAAB"
