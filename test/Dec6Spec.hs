module Dec6Spec where
  ...
  spec = describe "Dec 6" $ do
    ...
    context "parsing" $ do
      it "parses (1,2)" $ do
        coord "1, 2" `shouldBe` (1,2)

    context "labelling coords" $ do
      it "gives chords character ids" $ do
        points input `shouldBe` [(((1,1),['A'])), (((1,6),['B'])), (((8,3),['C'])), (((3,4),['D'])), (((5,5),['E'])), (((8,9),['F']))]
    ...
    context "updating for all coords for a single candidate" $ do
      it "writes A to all empty positions, but does not overwrite other coordinates" $ do
        let correct = "AAAAAAAAAA\nAAAAAAAAAA\nAAAAAAAAAA\nAAAAAAAACA\nAAADAAAAAA\nAAAAAEAAAA\nABAAAAAAAA\nAAAAAAAAAA\nAAAAAAAAAA\nAAAAAAAAFA\n"
    ...
    context "unmarking stuff" $ do
      ...
      it "can find edges" $ do
        infinites g' `shouldBe` ['A','C', 'B', 'F']
    ...
    context "full example for a" $ do
      ...
      it "reports area size" $ do
        sizeOf (g') 'E' `shouldBe` 17

      it "solves A" $ do
        solveA input `shouldBe` "17"
