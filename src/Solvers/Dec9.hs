...
module Solvers.Dec9 where
  ...
  newtype MarbleList a = ML ([a], [a]) deriving (Eq, Show)
  ...
  initial (ps, lm) = State { marbles = insert 0 $ empty, toBePlaced = 1, toPlay = 0, scores = replicate ps 0, lastMarble = lm }
  ...
  place :: State -> State
  place s = s {
      marbles = insert (toBePlaced s) . right 2 . marbles $ s,
  ...
  solve n = show . maximum . scores . head . filter gameOver . iterate play . scale . initial . parse
