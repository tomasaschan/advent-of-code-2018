module Solvers.Dec16 where
  ...
  solveA = show . fmap (length . filter (>=3) . fmap matchCount . fst) . parse parseInput "input" . unlines

  solveB :: [String] -> String
  solveB input = either show show $ do
    parsed <- parse16 input
    let opCodes = refine IntMap.empty . foldl (flip learn) anythingsPossible . fst $ parsed
  ...
  interpret :: IntMap Op -> (Int, Int, Int, Int) -> Either String (Op, Int, Int, Int)
  interpret codes (o,x,y,z) =
    case IntMap.lookup o codes of
      Just op -> Right (op, x, y, z)
      Nothing -> Left $ "the interpretation of " ++ show o ++ " was not determined by the learning steps"
  ...
  learn s = IntMap.adjust (\k -> k `Set.difference` (excludedPossibilities s)) (opCode s)
  ...
  refine known partials
    ...
    | otherwise =
      let
        determined = IntMap.map (Set.elemAt 0) . IntMap.filter ((==1) . Set.size) $ partials
        known' = known `IntMap.union` determined
        partials' = IntMap.map (flip Set.difference $ Set.fromList . IntMap.elems $ known') partials
