...
module Solvers.Dec10 where
  ...
  pretty :: [Star] -> String
  pretty sky = unlines [ [ marker x y | x <- [xmin..xmax] ] | y <- [ymin..ymax] ]
    where
      positions = Set.fromList . fmap getpos $ sky
      (xmin,xmax,ymin,ymax) = foldl folder (maxBound,minBound,maxBound,minBound) . fmap getpos $ sky
        where folder (xlo,xhi,ylo,yhi) (x,y) = (min xlo x, max xhi x, min ylo y, max yhi y)
      marker x y = if (x,y) `Set.member` positions then '#' else '.'

  simulate :: (Int, [Star]) -> (Int, [Star])
  simulate (t, sky)
    | height next < height sky = simulate (t + 1, next)
    | otherwise = (t, sky)
      where
        next = step sky
        step :: [Star] -> [Star]
        step sky' = fmap move sky' where move ((x,y), (vx,vy)) = ((x+vx, y+vy), (vx, vy))
  ...
  parse input = Parse.extract (particle . fmap read) parsed
    where
      ...
      parsed = Parse.expect "position=<" input
        >>= Parse.number
        >>= Parse.expect ", "
        >>= Parse.number
