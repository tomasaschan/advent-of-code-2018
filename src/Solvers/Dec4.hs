module Solvers.Dec4 where
  ...
  solveB input = show answer
    where
      ...
      answer = uncurry (*) $ (id >< fst) $ maximumBy (compare `on` (snd . snd)) $ map mapper guards

  type Guard = Int
  data State = Awake | Asleep | Away deriving (Eq, Show)
  type Watch = (Guard, [State])
  data EventType = BeginShift Guard | WakeUp | FallAsleep deriving (Show, Eq)

  data Event = Splitter | Event { month :: Int, day :: Int, hour :: Int, minute :: Int, what :: EventType } deriving (Show, Eq)
  ...
  buildEvent [mo,dd,hh,mm,"wakes up"] = Event { month = read mo, day = read dd, hour = read hh, minute = read mm, what = WakeUp }
  buildEvent [mo,dd,hh,mm,"falls asleep"] = Event { month = read mo, day = read dd, hour = read hh, minute = read mm, what = FallAsleep }
  buildEvent [mo,dd,hh,mm,guard] = Event { month = read mo, day = read dd, hour = read hh, minute = read mm, what = BeginShift $ read guard }
  ...
  _buildRow Away now (Event {
    minute = m,
    what = BeginShift _
  }:es) = replicate (m - now) Away ++ _buildRow Awake m es
  _buildRow Awake now (Event {
    minute = m,
    what = FallAsleep
  }:es) = replicate (m - now) Awake ++ _buildRow Asleep m es
  _buildRow Asleep now (Event {
    minute = m,
    what = WakeUp
  }:es) = replicate (m - now) Asleep ++ _buildRow Awake m es
  _buildRow s now [] = replicate (60-now) s
  _buildRow s _ (Event { what = w }:_) = error $ "invalid state transition from " ++ show s ++ " to " ++ show w
  _buildRow _ _ (Splitter:_) = error $ "Splitter not removed correctly"

  buildRow :: [Event] -> Watch
  buildRow (e:es) = case (what e) of
    BeginShift guard -> (guard, _buildRow Away 0 (e:es))
    _ -> error $ "Watch must begin with BeginShift event (this one started with " ++ show (what e) ++ ")"
  ...
  guardWithMostMinutesAsleep = fst . maximumBy (compare `on` snd) . Map.toList . minutesByGuard
