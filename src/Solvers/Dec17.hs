...
solveA = either show draw . fmap mapUnderground . foldAll . fmap (parse vein "vein")

draw :: Set (Int,Int) -> String
draw underground =
  let
    ((minx, miny), (maxx, maxy)) = mapSize underground
    xs = [minx-1..maxx+1]
    ys = [(min 0 miny)..maxy]

    square 500 0 = '+'
    square x y = if (x,y) `elem` underground then '#' else '.'
  ...
  in
    unlines $ [row100, row10, row1] ++ fmap line ys
