...
where
  ...
  borrowTree (Zero : ts)  = let (Node _ t1 t2, tss) = borrowTree ts in (t1, One t2 : tss)
  ...
  update f i (RList (One t : ts))
    ...
    | otherwise  = let RList rest = update f (i - size t) (RList ts) in RList $ One t : rest
    where
