...
bfsMapTraversal :: (Ord a, Ord p) => (a -> Bool) -> (a -> Seq a) -> (Seq a -> p) -> a -> Set a -> Maybe (Seq a)
bfsMapTraversal walkable neigbors prio start ends = _bfs Set.empty (Q.singleton (Seq.singleton $ start) (prio . Seq.singleton $ start))
  where
    _bfs seen q =
      case Q.minView q of
        Nothing -> Nothing
    ...
    _insertMany :: (Ord p, Ord a) => (Seq a -> p) -> PSQ (Seq a) p -> Seq (Seq a) -> PSQ (Seq a) p
    _insertMany p q Empty = q
    _insertMany p q (path:<|paths) =
      let q' = Q.insert path (p path) q
      in _insertMany p q' paths
