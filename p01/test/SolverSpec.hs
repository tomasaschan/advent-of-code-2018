...
spec =
  describe "1 Dec" $ do
    context "examples for a" $ do
      ...
      it "+1, +1, +1 =>  3" $ do solveA ["+1", "+1", "+1"] `shouldBe` "3"
      it "+1, +1, -2 =>  0" $ do solveA ["+1", "+1", "-2"] `shouldBe` "0"
      it "-1, -2, -3 => -6" $ do solveA ["-1", "-2", "-3"] `shouldBe` "-6"
    context "examples for b" $ do
      it "+1, -2, +3, +1 first reaches 2 twice" $ do
        solveB ["+1", "-2", "+3", "+1"] `shouldBe` "2"
      it "+1, -1 first reaches 0 twice." $ do solveB ["+1", "-1"] `shouldBe` "0"
